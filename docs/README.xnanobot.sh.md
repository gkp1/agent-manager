# 🐈 Nanobot Helper Script - Guia Docker Multi-Tenant

Script bash com **menu interativo** que automatiza o [Guia Docker Multi-Tenant](nanobot.md) para rodar múltiplas instâncias isoladas do Nanobot em containers Docker.

> **Não precisa memorizar comandos!** Execute o script e use o menu numérico.

## Visão Geral

O objetivo é ter **cada instância do nanobot rodando em Docker isolado**, com:
- `config.json` próprio
- `workspace/` próprio (memória isolada)
- `bridge/` próprio (sessão WhatsApp)
...
```bash
# Apenas execute o script - o menu aparece automaticamente!
./nanobot-helper.sh
```

O menu mostra todas as opções numeradas. É só escolher o número:

```
╔══════════════════════════════════════════════════════════════════╗
║    🐈 Nanobot Helper - Menu Principal                           ║
║    Instâncias Docker Multi-Tenant Isoladas                       ║
╚══════════════════════════════════════════════════════════════════╝

━━━ Configuração Rápida ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  [1] Configurar Tudo (Recomendado)  - Pré-requisitos + Imagem + Instância
  [2] Verificar Pré-requisitos       - Docker 20.10+, Compose 2.0+
  [3] Construir Imagem Docker         - Clonar repo e buildar

━━━ Criar Instâncias ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  [4] Criar Nova Instância            - WhatsApp, Telegram, Discord...
  [5] Criar Múltiplas Instâncias      - Várias de uma vez
  [6] Listar Instâncias Existentes    - Ver todas as instâncias

... (mais opções)

Escolha uma opção [0-20]: _
```

### Opção 2: Comandos Diretos (Avançado)

Se você já conhece os comandos, pode usar diretamente:

```bash
./nanobot-helper.sh setup-guide    # Configuração completa
./nanobot-helper.sh create         # Criar instância
./nanobot-helper.sh start wa1      # Iniciar instância
```

## Fluxo Completo (Conforme Guia)

### Pré-requisitos

```bash
# Verificar se Docker e Docker Compose estão instalados
./nanobot-helper.sh check
```
...
### Passo 1: Configuração Completa (Recomendado)

```bash
# Executa tudo automaticamente:
# 1. Verifica pré-requisitos
# 2. Clona repositório e constrói imagem Docker
# 3. Cria primeira instância Docker isolada
./nanobot-helper.sh setup-guide
```
...
./nanobot-helper.sh build

# 2. Criar instância
./nanobot-helper.sh create

# 3. Conectar WhatsApp (se for WhatsApp)
./nanobot-helper.sh login wa1

# 4. Iniciar instância
./nanobot-helper.sh start wa1
```

### Passo 2: Construir Imagem Docker (Manual)

```bash
# Clona repositório e constrói imagem
./nanobot-helper.sh build
```
...
### Passo 3: Criar Instância Docker
...
- **Nome da instância**: wa1, wa2, telegram1, discord1, etc.
- **Canal**: whatsapp, telegram, discord, etc.
- **Porta externa**: 18791, 18792, 18793, etc.
- **Chave API OpenRouter**: sk-or-v1-...
- **ID do usuário**: +5511999999999, SEU_USER_ID, etc.
- **Modelo**: x-ai/grok-4.1-fast, anthropic/claude-sonnet-4-6, etc.

### Passo 4: Conectar WhatsApp (Apenas para WhatsApp)
...
### Passo 5: Iniciar Instância

```bash
./nanobot-helper.sh start wa1
```

## Estrutura Exata (Conforme Guia)

```
~/nanobot-instances/
├── wa1/                    # WhatsApp Instância 1
│   ├── docker-compose.yml
│   ├── config.json
│   ├── bridge/             # Sessão WhatsApp (QR code) - criado após login
│   └── workspace/          # Memória e sessões - criado automaticamente
├── wa2/                    # WhatsApp Instância 2
│   ├── docker-compose.yml
│   ├── config.json
│   ├── bridge/
│   └── workspace/
...
## Configurações Exatas do Guia

### Config.json WhatsApp (wa1)
...
## Comandos por Instância (Conforme Guia)
...
# Parar
./nanobot-helper.sh stop wa1

# Ver logs
./nanobot-helper.sh logs wa1

# Status
./nanobot-helper.sh status wa1

# Chat CLI
./nanobot-helper.sh chat wa1

# Reconectar (trocar conta)
./nanobot-helper.sh reconnect wa1
```
...
```bash
# Iniciar
./nanobot-helper.sh start telegram1

# Status
./nanobot-helper.sh status telegram1

# Chat CLI
./nanobot-helper.sh chat telegram1
```
...
```bash
# Iniciar
./nanobot-helper.sh start discord1

# Status
./nanobot-helper.sh status discord1
```

## Criar Múltiplas Instâncias

```bash
# Criar 3 instâncias WhatsApp de uma vez
./nanobot-helper.sh create-multi

# Quantas: 3
# Prefixo: wa
# Canal: whatsapp
# Porta inicial: 18791
# Chave API: sk-or-v1-sua-chave
# Modelo: 1 (anthropic/claude-sonnet-4-6)

# Resultado:
# ~/nanobot-instances/wa1/ (porta 18791)
# ~/nanobot-instances/wa2/ (porta 18792)
# ~/nanobot-instances/wa3/ (porta 18793)
```
...
./nanobot-helper.sh start-all

# Parar todas as instâncias
./nanobot-helper.sh stop-all

# Reiniciar todas as instâncias
./nanobot-helper.sh restart-all

# Atualizar todas as instâncias
./nanobot-helper.sh update-all
```

## Solução de Problemas (Conforme Guia)
...
ports:
  - "18801:18790"  # Mudar de 18791 para 18801
```
...
docker compose down
rm -rf bridge
docker compose run --rm --entrypoint nanobot nanobot channels login whatsapp
docker compose up -d
```
...
./nanobot-helper.sh rebuild
```

### Reconectar WhatsApp (Trocar Conta)
...
```bash
# Atualizar nanobot (pip/uv)
./nanobot-helper.sh update
...
## Template para Criar Nova Instância Manualmente
...
```json
{
  ...
  "channels": {
    "whatsapp": {
      "enabled": true,
      "allowFrom": ["+5511XXXXXXXXX"]
    }
  }
}
```

Substitua `NUM` (1, 2, 3...) e `PORTA_EXTERNA` (18791, 18792, 18793...).

> **Lembrete:** Porta interna sempre é `18790`. Só muda a porta externa.

## Menu Interativo - Opções

| Número | Opção | Descrição |
|--------|-------|-----------|
| **1** | Configurar Tudo | Pré-requisitos + Imagem + Instância (Recomendado!) |
| **2** | Verificar Pré-requisitos | Docker 20.10+, Compose 2.0+ |
| **3** | Construir Imagem Docker | Clonar repo e buildar |
| **4** | Criar Nova Instância | WhatsApp, Telegram, Discord... |
| **5** | Criar Múltiplas Instâncias | Várias de uma vez |
| **6** | Listar Instâncias | Ver todas as instâncias |
| **7** | Iniciar Instância | Escolhe qual instância iniciar |
| **8** | Parar Instância | Escolhe qual instância parar |
| **9** | Ver Logs | Ver logs de uma instância |
| **10** | Status | Verificar se instância está rodando |
| **11** | Chat CLI | Abrir chat interativo com instância |
| **12** | Conectar WhatsApp | QR Code para parear |
| **13** | Reconectar WhatsApp | Trocar conta ou reconectar |
| **14** | Iniciar Todas | Start em lote |
| **15** | Parar Todas | Stop em lote |
| **16** | Atualizar Todas | Reiniciar com nova imagem |
| **17** | Atualizar Nanobot | pip/uv update |
| **18** | Reconstruir Imagem | Build após update |
| **19** | Configurar Canal | Instruções por canal |
| **20** | Ajuda | Mostrar comandos |
| **0** | Sair | Encerrar |

## Comandos Diretos (Avançado)

Se preferir usar comandos sem o menu:
...
| `create-multi` | Criar múltiplas instâncias |
| `list` | Listar instâncias |
| `start <name>` | Iniciar instância |
| `stop <name>` | Parar instância |
| `logs <name>` | Ver logs |
| `status <name>` | Status da instância |
| `chat <name>` | Chat CLI |
| `login <name>` | Conectar WhatsApp |
| `reconnect <name>` | Reconectar WhatsApp |
| `start-all` | Iniciar todas |
| `stop-all` | Parar todas |
| `update-all` | Atualizar todas |
| `update` | Atualizar nanobot |
| `rebuild` | Reconstruir imagem |
| `configure <ch>` | Configurar canal |
| `help` | Mostrar ajuda |

## Padrão para N Instâncias

| Instância | Pasta | Porta | Container |
|-----------|-------|-------|-----------|
| wa1 | `~/nanobot-instances/wa1/` | 18791 | `nanobot-wa1` |
| wa2 | `~/nanobot-instances/wa2/` | 18792 | `nanobot-wa2` |
| wa3 | `~/nanobot-instances/wa3/` | 18793 | `nanobot-wa3` |
| wa4 | `~/nanobot-instances/wa4/` | 18794 | `nanobot-wa4` |
| wa5 | `~/nanobot-instances/wa5/` | 18795 | `nanobot-wa5` |
| telegram1 | `~/nanobot-instances/telegram1/` | 18796 | `nanobot-telegram1` |
| discord1 | `~/nanobot-instances/discord1/` | 18797 | `nanobot-discord1` |
...
- **Git**: Qualquer versão
- **Recursos**: 2GB RAM mínimo por instância
- **Portas**: Portas únicas para cada instância (18791, 18792, etc.)

## Dicas para Iniciantes

1. **Primeira vez?** Execute `./nanobot-helper.sh` e escolha `[1] Configurar Tudo`
2. **Não sabe o que escolher?** O script guia você com perguntas
3. **Errou algo?** Sempre pode recomeçar, as configurações ficam em `~/nanobot-instances/`
4. **Precisa de ajuda?** Escolha `[20] Ajuda Detalhada` no menu
5. **Quer ver o que tem?** Escolha `[6] Listar Instâncias`

## Dicas Úteis

1. **Portas**: Cada instância precisa de porta externa única (18791, 18792, etc.)
2. **API Keys**: Use OpenRouter para acesso a múltiplos modelos
3. **WhatsApp**: A sessão (bridge) é salva localmente, não perca a pasta
4. **Memória**: Cada instância tem workspace próprio (memória isolada)
5. **Updates**: Após atualizar nanobot, recrie instâncias WhatsApp (reconnect)
6. **Logs**: Use `./nanobot-helper.sh logs <nome>` para debug
7. **Modelos**: wa1 usa `x-ai/grok-4.1-fast`, wa2 usa `anthropic/claude-sonnet-4-6` no guia

---

*Baseado no [Guia Docker Multi-Tenant](nanobot.md) - Atualizado em Março 2026*
